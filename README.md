...
> 受限于个人能力与项目经验，当前实现仍有许多可以改进的地方，包括代码实现、架构设计和功能规划等方面。欢迎提出 Issue、建议或优化思路，帮助项目持续完善。
...
task.addObserver(new TaskObserver() {
    ...
    public void onStateChanged(BaseDownloadTask t, TaskState state) {
        System.out.println("状态变更: " + state);
    }
    @Override
    public void onProgressUpdate(BaseDownloadTask t, DownloadProgress p) {
        System.out.printf("进度: %d/%d 图片%n", p.completedImages(), p.totalImages());
    }
    @Override
    public void onFinished(BaseDownloadTask t, DownloadResult result) {
        System.out.println("完成: " + result.getSuccessfulFiles().size() + " 张");
    }
    @Override
...
完整文档请访问：[JMComic-Api-Java 文档](https://jmcomic-api-java.readthedocs.io)
